...
WITH WindAndSolarProcessing AS (
    ...
    FROM [SARAH-ERA5].[dbo].[PVGIS_REZoning]

	---------------------------------------------
	WHERE Year = 2013 
	---------------------------------------------

--	WHERE country in ('Greece','France','Spain','Portugal','Bulgaria','Poland','Finland','Sweden','Denmark')
),
...
FROM FinalFractions
WHERE Year = 2013 
ORDER BY grid_cell, Year, Month, Day, Hour;
