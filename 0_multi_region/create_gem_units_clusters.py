...
class GEMUnitsClusterer:
    ...
    def load_gem_data(self, gem_file='../data/existing_stock/Global-Integrated-Power-April-2025.xlsx'):
        ...
        # Filter for operational and under construction plants only
        if 'Status' in country_data.columns:
            operational_statuses = [
                # Title case variants
                'Operating', 'Operational', 'Active', 'Online', 'In Operation',
                'Under construction', 'Construction', 'Under Construction',
                # Lowercase variants (actual GEM data format)
                'operating', 'operational', 'active', 'online', 'in operation',
                'under construction', 'construction', 'under_construction'
            ]
            initial_count = len(country_data)
            status_filter = country_data['Status'].isin(operational_statuses)
            country_data = country_data[status_filter]
            filtered_count = len(country_data)
            print(f"Filtered to operational/under construction: {filtered_count}/{initial_count} plants")
