...
const CapacityChart = ({ countryIso, year = 2022, height }) => {
  ...
  const getChartOptions = () => {
    if (!chartData || !chartData.capacity_chart) {
      console.warn("CapacityChart: Missing required data properties");
      // Return a basic chart structure even with no data
      return createAreaChartConfig({
        title: "No Capacity Data Available",
        subtitle: "Installed capacity by fuel type (2000-2022)",
    ...
    // Check if capacity_chart has actual data (not empty object)
    const hasCapacityData =
      chartData.capacity_chart &&
      Object.keys(chartData.capacity_chart).length > 0 &&
      Object.values(chartData.capacity_chart).some(
        (arr) => arr && arr.length > 0
      );

    if (!hasCapacityData) {
      console.warn(
        "CapacityChart: No capacity data available for this country"
      );
    ...
    const { capacity_chart, fuel_types, years } = chartData;

    // Use fuel_types and years from backend, fallback to extracting from capacity_chart
    const fuelTypes = fuel_types || Object.keys(capacity_chart);
    const chartYears = years || Array.from({ length: capacity_chart[fuelTypes[0]]?.length || 0 }, (_, i) => 2000 + i);

    // Create series data for each fuel type
    const series = fuelTypes.map((fuelType) => {
      const fuelData = capacity_chart[fuelType] || [];
    ...
    return (
      <div className="flex items-center justify-center h-96 bg-gray-50 rounded-lg">
        <div className="text-center">
          ...
          <button onClick={loadCapacityData} className="btn-primary">
            Try Again
          </button>
        </div>
      </div>
